...
def save_task(task): 
    ...
    try:
        with conn.cursor() as cur:
            print(f"title={task['title']}, date={task['date']}, tipo={task['tipo']}, phone={task['phone']}")
            cur.execute(
                """
                ...
                RETURNING id
                """,
                (task["title"], task["date"], task["tipo"], task["phone"])
            )
...
def get_tasks(tel):
    ...
    try:
        with conn.cursor() as cur:
            ...
            rows = cur.fetchall()
            return [
                {"id": r[0], "title": r[1], "date": r[2], "tipo": r[3], "phone": r[4]}
                for r in rows
            ]
    except Exception as e:
        raise e
