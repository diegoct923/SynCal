...
from shared.services.service import create_user_task, complete_task, delete_task, reagendar_user_task
from shared.storage.grupo_store import crear_grupo, get_grupos_usuario, save_group_task
from shared.storage.task_store import get_tasks, get_tasks_day, get_tasks_numbered, get_real_task_id_from_user_number
from shared.storage.user_store import create_user_if_not_exists, set_minutos_anticipacion_to_notify        
from shared.config.config import BASE_URL
from shared.utils.state import generar_state
from shared.utils.parser import parse_task_data, parse_intent, parsear_con_llm, parse_grupo_data, parse_anticipacion, extract_date, extract_time
from shared.storage.sesiones import guardar_state
from shared.storage.conversacion import obtener_contexto, limpiar_contexto, guardar_contexto
...
def webhook():
    ...
    if contexto == "id_tarea_completar":
        numero = incoming_msg.strip()   #type: ignore

        if not numero.isdigit():
            response.message("Por favor respondé con el número de la tarea.")
            return str(response)

        task_id = get_real_task_id_from_user_number(tel, int(numero))

        if task_id is None:
            limpiar_contexto(tel)
            response.message("No encontré una tarea con ese número.")
            return str(response)
    ...
    if contexto == "id_tarea_mover":
        ...
        if len(parts) < 2:
            response.message("Por favor respondé con el formato: número, nueva fecha")
            return str(response)

        numero = parts[0].strip()
        fecha_texto = parts[1].strip()
        ...
        fecha = extract_date(fecha_texto)
        hora = extract_time(fecha_texto)

        if fecha is None:
            response.message("Por favor incluí una fecha válida.")
            return str(response)

        deadline = datetime.combine(fecha, hora) if hora else datetime.combine(fecha, time.min)

        if deadline.date() < date.today():
            response.message("La fecha ingresada es anterior al día del registro.")
            return str(response)
    ...
    if contexto == "id_tarea_eliminar":
        ...
        result = delete_task(task_id, tel)
    ...
    # COMPLETE
        ...
        if not tasks:
            ...
            for t in tasks:
                msg += f"{t['numero_usuario']} - {t['tipo']} {t['title']} ({t['deadline']})\n"
