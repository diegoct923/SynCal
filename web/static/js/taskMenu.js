...
changeDateTaskBtn.addEventListener("click", (e) => {
  ...
  openCustomModal(
    ...
    `
      ...
      <input id="taskHourInput" type="time" step="1800" value="${formatHour(task.startHour ?? 9)}">
    ...
    () => {
      ...
      const newHour = timeToDecimal(hourInput.value);
      ...
      if (!hourInput.value || Number.isNaN(newHour) || newHour < 0 || newHour > 23.5) {
      ...
      (async () => {
          try {
              await fetch(`${BASE_URL}/api/tasks/reagendar`, {
                  ...
                  body: JSON.stringify({
                      task_id: taskId,
                      deadline: buildDeadline(newDate, newHour),
                      state: getState()
                  })
              });
              //location.reload();
