from shared.storage.task_store import completar_tarea, save_task, borrar_sesiones, reagendar_tarea, borrar_tarea
from shared.scheduler.scheduler import *
...
def create_user_task(phone, tipo, title, date, es_grupal = False):
    ...
    if res["status"] == "inserted" and tipo in ("EXAMEN", "TAREA") and not es_grupal:
        ...
        celery_app.send_task(
            "shared.celery_tasks.planificar_tarea_task",
            args=[res["task"]["id"], phone], #type:ignore
        )  # async, no bloquea
